...
cache = {
    "sonuc": None,
    "guncelleme": None,
    "durum": "bekleniyor",  # bekleniyor / taraniyor / tamamlandi
    "taranan": 0,
    "toplam": 0
}
...
def analiz_et(ticker, lookback=50):
    ...
    b_up = pma+atr*avg_ra; b_mu = pma+atr*avg_rb
    b_md = pma-atr*avg_rb; b_lo = pma-atr*avg_ra
    ...
    rsi_al=rsi<70; rsi_sat=rsi>35; adx_ok=adx>=18
    di_bull=dip>dim; di_bear=dim>dip
    ...
    sinyal_tip="BEKLE"; sinyal_guclu=False; sinyal_skor=0
    tp_sev=None; sl_sev=None
    ...
    return {
        ...
        "guclu":sinyal_guclu,"skor":sinyal_skor,
...
# ─── ARKA PLAN TARAMA ─────────────────────────────────────────────────
def arka_plan_tara():
    """Tüm BIST'i yavaşça tara — rate limit'e takılma"""
    global cache
    while True:
        with cache_lock:
            cache["durum"] = "taraniyor"
            cache["taranan"] = 0
            cache["toplam"] = len(BIST_TUMU)
        ...
        for h in BIST_TUMU:
            ...
            # Rate limit önleme — her hisse arasında 0.5 sn bekle
            time.sleep(0.5)
...
def anasayfa():
    with cache_lock:
        durum = cache["durum"]
        taranan = cache["taranan"]
        toplam = cache["toplam"]
        guncelleme = cache["guncelleme"]
    return jsonify({
        "sistem": "Pisagor PRO API", "versiyon": "6.0",
        "toplam_hisse": len(BIST_TUMU),
        "tarama_durumu": durum,
        "taranan": f"{taranan}/{toplam}",
        "son_guncelleme": guncelleme,
        "endpointler": {
            "/tarama": "Son tarama sonuçları (anında döner)",
...
def tarama():
    ...
    if sonuc is None:
        return jsonify({
            "durum": durum,
            "mesaj": f"İlk tarama devam ediyor... {taranan}/{toplam} hisse tamamlandı",
            "sinyal_sayisi": 0,
            "sinyaller": []
        })
...
if __name__ == "__main__":
    ...
    t = threading.Thread(target=arka_plan_tara, daemon=True)
    t.start()
    app.run(host="0.0.0.0", port=5000, debug=False)
else:
    # Gunicorn ile çalışırken de thread başlat
    t = threading.Thread(target=arka_plan_tara, daemon=True)
    t.start()
